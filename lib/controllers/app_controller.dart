...
class AppController extends ChangeNotifier {
  ...
  final Map<String, int> cart = {'chicken-biryani': 1, 'gulab': 2};
  ...
  void openChat({String? topic}) {
    trackMenuOpen = false;
    if (chatMessages.isEmpty) {
      chatMessages.add(ChatMessage(
        text: "Hi ${store.userName.split(' ').first}! 👋 How can I help you with your order from ${restaurant.name}?",
        fromCustomer: false,
        time: _timeNow(),
      ));
    }
    if (topic != null) {
      ...
      createSupportFromChat(topic);
      _replyAfterDelay(store.agentRepliesByTopic[topic] ?? store.defaultAgentReply);
    }
    push('chat');
  }
  ...
  void sendChatMessage(String text) {
    ...
    notifyListeners();
    createSupportFromChat(text.trim());
    _replyAfterDelay(store.defaultAgentReply);
  }

  void _replyAfterDelay(String reply) {
    agentTyping = true;
    notifyListeners();
    Future.delayed(const Duration(milliseconds: 1100), () {
      agentTyping = false;
      chatMessages.add(ChatMessage(text: reply, fromCustomer: false, time: _timeNow()));
      notifyListeners();
    });
  }
